...
export default function App() {
  ...
  const fetchUserProfile = async (userId) => {
    ...
    if (data) {
      ...
      setCurrentView((prevView) =>
        prevView === "login" ? "dashboard" : prevView,
      );
      fetchData();
    }
    setIsLoading(false);
  ...
  const fetchData = async () => {
    ...
    supabase
      ...
      .select("*")
      .then(({ data }) => data && setEnrollments(data));
    supabase
      .from("edu_results")
      .select("*")
  ...
  const handleAuth = async () => {
    if (!authForm.email || !authForm.password)
      return showToast("Vui lòng điền đủ Email và Mật khẩu!", "error");
    setIsLoading(true);
    try {
      if (authMode === "login") {
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 flex flex-col">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-40 shrink-0 shadow-sm no-print animate-fade-in-down">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div
            className="flex items-center gap-2 font-black text-xl cursor-pointer tracking-tight hover:scale-105 transition-transform"
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="text-gray-500 hover:text-red-500 transition-colors p-2 rounded-xl hover:bg-red-50 border border-transparent hover:border-red-100"
              title="Đăng xuất"
            >
              <LogOut size={20} />
            </button>
      ...
      <footer className="py-6 text-center text-sm font-bold text-gray-400 no-print border-t border-gray-200 mt-auto bg-gray-50/80">
        <p>
          ...
          <span
            className="text-black font-black hover:underline cursor-pointer transition-colors"
            title="Nhà phát triển hệ thống"
          >
            Hong Quang Tech
          </span>
        </p>
        <p className="text-xs mt-1.5 font-medium">
          EduSheet © 2026 - Nền tảng Giáo dục Tối giản
        </p>
      </footer>
...
function Toast({ toast }) {
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 ${bgColors[toast.type]} text-white px-6 py-4 rounded-2xl shadow-2xl z-[200] animate-toast font-bold flex items-center gap-3 border border-white/20`}
    >
      {toast.type === "success" ? (
        <CheckCircle size={20} />
      ) : toast.type === "error" ? (
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[200] p-4 backdrop-blur-sm transition-opacity">
      <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl animate-pop text-center">
        ...
        <h3 className="text-xl font-black mb-6 text-gray-800 leading-tight">
          {confirmDialog.message}
        </h3>
        <div className="flex gap-3 justify-center">
          <button
            onClick={() => setConfirmDialog(null)}
            className="px-6 py-3 rounded-xl bg-gray-100 font-bold hover:bg-gray-200 transition-colors flex-1"
          >
            Hủy
          </button>
          <button
            onClick={confirmDialog.onConfirm}
            className="px-6 py-3 rounded-xl bg-red-500 text-white font-bold hover:bg-red-600 transition-colors flex-1 shadow-md"
          >
            Đồng ý
          </button>
        </div>
...
function Dashboard({
  ...
  // ----------------------------------------------------
  const myResults =
    results?.filter((r) => r.student_id === currentUser.id) || [];
  const totalExamsTaken = myResults.length;
  const avgScore =
    totalExamsTaken > 0
      ? (
          myResults.reduce(
            (acc, curr) => acc + (curr.score / curr.total) * 10,
            0,
          ) / totalExamsTaken
        ).toFixed(1)
      : 0;
  ...
  const studentStats =
    allUsers
      ...
      .map((student) => {
        const stuResults =
          results?.filter((r) => r.student_id === student.id) || [];
        const avg =
          stuResults.length > 0
            ? stuResults.reduce(
                (acc, curr) => acc + (curr.score / curr.total) * 10,
  ...
  const myClasses =
    currentUser.role === "teacher"
      ...
      : classes.filter((c) =>
          enrollments.some(
            (e) => e.class_id === c.id && e.student_id === currentUser.id,
          ),
        );
  ...
  const uniqueStudentsCount = new Set(
    enrollments
      .filter((e) => myClassIds.includes(e.class_id))
      .map((e) => e.student_id),
  ).size;
  // Tính tổng số đề thi đã tạo (không tính đề bị ẩn)
  const teacherExamsCount = exams.filter(
    (e) => myClassIds.includes(e.classId) && !e.is_hidden,
  ).length;
  ...
  const handleCreateClass = async () => {
    if (!newClassName.trim())
      return showToast("Vui lòng nhập tên lớp!", "error");
  ...
  const handleJoinClass = async (codeToJoin = joinCode) => {
    ...
    const alreadyJoined = enrollments.some(
      (e) => e.class_id === found.id && e.student_id === currentUser.id,
    );
    if (alreadyJoined) {
  ...
  const handleDeleteClass = (e, classObj) => {
    e.stopPropagation();
    showConfirm(
      `CẢNH BÁO: Bạn có chắc chắn muốn xóa vĩnh viễn lớp "${classObj.name}" không?\nMọi dữ liệu liên quan sẽ bị ẩn đi.`,
      async () => {
          ...
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl animate-pop">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-6 py-3 rounded-xl bg-gray-100 font-bold hover:bg-gray-200 transition-colors"
              >
                Hủy
              </button>
              <button
                onClick={handleCreateClass}
                className="px-6 py-3 rounded-xl bg-black text-white font-bold hover:bg-gray-800 transition-transform hover:scale-105 active:scale-95"
              >
                Tạo Lớp
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white border border-gray-200 rounded-3xl p-6 text-gray-900 shadow-sm relative overflow-hidden group hover:shadow-md hover:-translate-y-1 transition-all">
                <BarChart3
                  ...
                  size={80}
                />
                <h3 className="font-bold text-gray-500 mb-1 relative z-10">
                  Điểm Trung Bình
                </h3>
              ...
              <div className="lg:col-span-2 bg-white p-8 rounded-3xl border border-gray-200 shadow-sm">
                ...
                {chartData.length === 0 ? (
                  <div className="h-48 flex items-center justify-center text-gray-400 font-bold bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                    Bạn chưa làm bài nào. Khởi động ngay thôi!
                  </div>
                ) : (
                  <div className="flex items-end gap-3 sm:gap-6 h-56 mt-4 border-b-2 border-gray-100 pb-2 overflow-x-auto pt-8">
                    {chartData.map((r, i) => {
                      ...
                      const displayScore = ((r.score / r.total) * 10).toFixed(
                        1,
                      );
                      return (
                        <div
                          key={i}
                          className="flex-1 flex flex-col items-center justify-end group min-w-[40px] h-full"
                        >
                          <div className="opacity-0 group-hover:opacity-100 transition-opacity text-xs font-black mb-2 text-black bg-gray-100 px-2 py-1 rounded-md">
                            {displayScore}
                          </div>
                          <div
                            className="w-full max-w-[40px] bg-gradient-to-t from-gray-900 to-gray-700 rounded-t-xl transition-all duration-500 shadow-md group-hover:from-black group-hover:to-gray-800 relative overflow-hidden"
                            style={{ height: `${heightPercent}%` }}
                          >
                            <div className="absolute top-0 left-0 w-full h-2 bg-white/20"></div>
                          </div>
                          <div
                            className="text-[10px] mt-3 text-gray-400 font-bold truncate w-full text-center group-hover:text-black transition-colors"
                            title={examInfo?.title}
                          >
                            {examInfo?.title || "Bài thi"}
                          </div>
                        </div>
                      );
                    })}
                ...
                {topStudents.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {topStudents.map((s, index) => (
                      <div
                        key={s.id}
                        className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl hover:bg-gray-100 transition-colors border border-gray-100 relative overflow-hidden"
                      >
                        {index === 0 && (
                        ...
                        <div className="flex items-center gap-4">
                          <div
                            className={`font-black text-xl w-6 text-center ${index === 0 ? "text-black" : index === 1 ? "text-gray-400" : index === 2 ? "text-gray-400" : "text-gray-300"}`}
                          >
                            {index === 0 ? (
                              <Star className="fill-black inline" size={20} />
                            ) : (
                              index + 1
                            )}
                          </div>
                          <div>
                            <div className="font-black text-gray-800 text-sm sm:text-base truncate max-w-[120px]">
                              {s.name}
                            </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div className="flex items-center gap-2">
                  ...
                  {cheatIncidents.length > 0 && (
                    <span className="ml-2 bg-gray-100 text-black border border-gray-200 text-xs font-black px-2.5 py-1 rounded-full animate-pulse tracking-wide uppercase">
                      Có phát hiện mới
                    </span>
                  )}
                </div>
              ...
              {cheatIncidents.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-gray-400 py-6 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50">
                  ...
                  <p className="font-bold">
                    Không phát hiện học sinh nào có dấu hiệu gian lận.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {cheatIncidents.map((incident, i) => (
                    <div
                      key={i}
                      className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 bg-white rounded-2xl border border-gray-200 hover:border-gray-300 hover:shadow-sm transition-all gap-4"
                    >
                      <div className="flex items-start sm:items-center gap-4">
                        ...
                        <div>
                          <div className="font-bold text-gray-800 text-base">
                            Học sinh{" "}
                            <span className="text-black font-black bg-gray-100 px-2 py-0.5 rounded shadow-sm border border-gray-200 mx-1">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-t border-gray-200 pt-8">
          ...
          {currentUser.role === "teacher" ? (
            <button
              ...
              className="bg-black text-white px-5 py-2.5 rounded-xl flex items-center gap-2 font-bold hover:bg-gray-800 hover:-translate-y-1 transition-all shadow-md hover:shadow-lg"
            >
              ...
              {isJoining ? (
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  <button
                    ...
                    className="bg-black text-white px-6 py-2 rounded-xl font-bold hover:bg-gray-800 transition-transform hover:scale-105 active:scale-95"
                  >
                    Vào
                  </button>
                  <button
                    onClick={() => setIsJoining(false)}
                    className="px-5 py-2 rounded-xl bg-gray-100 font-bold hover:bg-gray-200 transition-colors"
                  >
                    Hủy
                  </button>
                </div>
                ...
                <button
                  ...
                  className="bg-white border-2 border-black text-black px-5 py-2.5 rounded-xl flex items-center gap-2 font-bold hover:bg-gray-50 hover:-translate-y-1 transition-all shadow-sm"
                >
          ...
          <div className="text-center py-16 bg-white rounded-3xl border border-gray-200 shadow-sm animate-pop">
            ...
            {currentUser.role === "student" && (
              <p className="text-gray-400 text-sm">
                Hãy xin mã lớp từ giáo viên hoặc bấm vào link chia sẻ để tham
                gia nhé!
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
            {myClasses.map((cls, idx) => (
              ...
              >
                ...
                <h3 className="text-xl font-black mb-2 truncate text-gray-800 pr-8">
                  {cls.name}
                </h3>
                {currentUser.role === "teacher" && (
                  <p className="text-gray-500 text-sm mb-5 font-medium">
                    Mã chia sẻ:{" "}
                    <span className="font-mono font-bold bg-gray-100 text-black px-2 py-1 rounded-md">
                      {cls.code}
                    </span>
                  </p>
                )}
...
function ClassDetail({
  ...
  const handleDeleteExam = (exam) => {
    showConfirm(
      `Bạn có chắc muốn xóa đề "${exam.title}" không? \nTệp PDF sẽ bị xóa vĩnh viễn để giải phóng dung lượng.`,
      async () => {
        ...
        await supabase
          .from("edu_exams")
          .update({ fileUrl: null, is_hidden: true })
          .eq("id", exam.id);
        setExams((prev) =>
          prev.map((e) =>
            e.id === exam.id ? { ...e, fileUrl: null, is_hidden: true } : e,
          ),
        );
  ...
  const handleSaveDeadline = async () => {
    await supabase
      ...
      .eq("id", extendingExam.id);
    setExams((prev) =>
      prev.map((e) =>
        e.id === extendingExam.id ? { ...e, deadline: newDeadline } : e,
      ),
    );
  ...
  const handleRemoveStudent = (student) => {
    showConfirm(
      `Bạn có chắc muốn mời học sinh "${student.name}" ra khỏi lớp không?`,
      async () => {
        await supabase
          .from("edu_enrollments")
          .delete()
          .match({ class_id: cls.id, student_id: student.id });
        setEnrollments((prev) =>
          prev.filter(
            (e) => !(e.class_id === cls.id && e.student_id === student.id),
          ),
        );
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4 backdrop-blur-sm transition-opacity no-print">
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl animate-pop">
            ...
            <p className="text-gray-500 font-bold mb-6 text-sm">
              Chỉnh sửa hạn nộp bài:{" "}
              <span className="text-black">{extendingExam.title}</span>
            </p>
      ...
      <div className="space-y-6 print-section page-transition">
        <button
          onClick={() => navigate("dashboard")}
          className="flex items-center text-sm text-gray-500 hover:text-black transition-colors font-bold no-print group"
        >
          <ArrowLeft
            size={16}
            ...
            {currentUser.role === "teacher" && (
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-gray-400 font-medium text-sm">
                  Mã tham gia:{" "}
                  <span className="text-white font-mono font-bold bg-white/20 px-3 py-1.5 rounded-lg ml-1">
                    {cls.code}
                  </span>
                </span>
                <button
                  onClick={copyLink}
                  className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg text-sm font-bold transition-colors no-print"
                >
        ...
        {currentUser.role === "teacher" && (
          <div className="flex gap-4 border-b border-gray-200 no-print">
            <button
              ...
              className={`pb-3 font-black text-lg border-b-4 transition-all duration-300 ${activeTab === "exams" ? "border-black text-black" : "border-transparent text-gray-400 hover:text-gray-600"}`}
            >
              Bài tập & Đề thi
            </button>
            <button
              onClick={() => setActiveTab("students")}
              className={`pb-3 font-black text-lg border-b-4 transition-all duration-300 flex items-center gap-2 ${activeTab === "students" ? "border-black text-black" : "border-transparent text-gray-400 hover:text-gray-600"}`}
            >
              Quản lý học sinh{" "}
              <span
                className={`text-xs px-2 py-0.5 rounded-full transition-colors ${activeTab === "students" ? "bg-black text-white" : "bg-gray-100 text-black"}`}
              >
          ...
          <div className="space-y-6 no-print animate-fade-in-down">
            {currentUser.role === "teacher" && (
              <div className="flex justify-end">
                <button
                  ...
                  className="bg-black text-white px-5 py-2.5 rounded-xl flex items-center gap-2 font-bold shadow-md hover:bg-gray-800 hover:-translate-y-1 transition-all"
                >
            ...
            <div className="bg-white rounded-[2rem] border border-gray-200 overflow-hidden shadow-sm">
              {visibleExams.length === 0 ? (
                <div className="p-16 text-center text-gray-400 font-bold text-lg">
                  Chưa có bài tập nào.
                </div>
              ) : (
                <div className="divide-y divide-gray-100">
                  {visibleExams.map((exam) => {
                    ...
                    const deadlineText = exam.deadline
                      ? new Date(exam.deadline).toLocaleString("vi-VN")
                      : "Không giới hạn";
                    const isExamMode = exam.exam_type === "exam";

                    const attemptsCount = results.filter(
                      (r) =>
                        r.exam_id === exam.id &&
                        r.student_id === currentUser.id,
                    ).length;
                    const hasReachedMaxAttempts =
                      exam.maxAttempts && attemptsCount >= exam.maxAttempts;

                    return (
                      <div
                        key={exam.id}
                        className="p-5 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-gray-50 transition-colors gap-4 group"
                      >
                        <div className="flex items-center gap-5">
                          <div
                            className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border group-hover:scale-110 transition-transform ${isExpired ? "bg-red-50 text-red-500 border-red-100" : isExamMode ? "bg-purple-50 text-purple-600 border-purple-200" : "bg-green-50 text-green-600 border-green-200"}`}
                          >
                            {isExpired ? (
                              <Clock size={28} />
                            ) : isExamMode ? (
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <h3 className="font-black text-lg text-gray-800 group-hover:text-black transition-colors">
                                {exam.title}
                              </h3>
                              {isExamMode ? (
                                <span className="text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded font-bold uppercase tracking-wider">
                                  Thi thật{" "}
                                  {exam.duration ? `(${exam.duration}p)` : ""}
                                </span>
                              ) : (
                            ...
                            <div className="flex gap-4">
                              ...
                              <p
                                className={`text-sm font-bold flex items-center gap-1 ${isExpired ? "text-red-500" : "text-blue-500"}`}
                              >
                        ...
                        <div className="flex gap-2">
                          {currentUser.role === "student" ? (
                            isExpired ? (
                              <button
                                disabled
                                className="bg-gray-200 text-gray-400 px-8 py-3 rounded-xl text-sm font-bold cursor-not-allowed"
                              >
                                Đã hết hạn
                              </button>
                              ...
                              <button
                                disabled
                                className="bg-red-100 text-red-500 px-6 py-3 rounded-xl text-sm font-bold cursor-not-allowed border border-red-200"
                              >
                                Đã hết {exam.maxAttempts} lượt
                              </button>
                            ) : (
                              <div className="flex flex-col items-end gap-1">
                                <button
                                  onClick={() =>
                                    navigate("exam_taker", { exam })
                                  }
                                  className="bg-black text-white px-8 py-3 rounded-xl text-sm font-bold hover:bg-gray-800 hover:scale-105 shadow-md transition-all w-full"
                                >
                                  Làm bài
                                </button>
                                {exam.maxAttempts && (
                                  <span className="text-[10px] text-gray-500 font-bold">
                                    Lượt: {attemptsCount}/{exam.maxAttempts}
                                  </span>
                                )}
                              </div>
                            )
                            ...
                            <>
                              <button
                                onClick={() => {
                                  ...
                                  setNewDeadline(exam.deadline || "");
                                }}
                                className="text-blue-600 font-bold flex items-center gap-1.5 bg-blue-50 px-4 py-2.5 rounded-xl hover:bg-blue-100 transition-all hover:scale-105"
                              ...
                              <button
                                ...
                                className="text-red-500 font-bold flex items-center gap-1.5 bg-red-50 px-4 py-2.5 rounded-xl hover:bg-red-100 transition-all hover:scale-105"
                                title="Xóa đề giải phóng dung lượng"
                              >
                                <Trash2 size={16} /> Xóa
                              </button>
                              <button
                                onClick={() => navigate("exam_taker", { exam })}
                                className="text-gray-600 font-bold flex items-center gap-1.5 bg-gray-100 px-4 py-2.5 rounded-xl hover:bg-gray-200 hover:text-black transition-all hover:scale-105"
                                title="Xem thử"
                              >
                                <Eye size={16} /> Xem
                              </button>
        ...
        {activeTab === "students" && currentUser.role === "teacher" && (
          <div className="space-y-6 animate-fade-in-down">
            <div className="flex justify-between items-center no-print">
              ...
              <button
                onClick={exportPDF}
                className="bg-white border-2 border-gray-200 px-4 py-2 rounded-xl flex items-center gap-2 hover:border-black hover:bg-gray-50 font-bold transition-all hover:-translate-y-0.5 shadow-sm"
              >
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50 text-gray-500 text-sm">
                      <th className="p-4 font-black border-b border-gray-200">
                        Học sinh
                      </th>
                      {exams.map((exam) => (
                        <th
                          key={exam.id}
                          className="p-4 font-black border-b border-gray-200 whitespace-nowrap text-center min-w-[120px]"
                        >
                          {exam.title}
                          <div className="flex justify-center gap-1 mt-1">
                            {exam.exam_type === "exam" && (
                              <span className="text-[10px] text-purple-500 bg-purple-50 px-1 rounded uppercase tracking-wider">
                                Thi thật
                              </span>
                            )}
                            {exam.is_hidden && (
                      ...
                      <th className="p-4 font-black border-b border-gray-200 text-right no-print">
                        Thao tác
                      </th>
                    </tr>
                  </thead>
                        ...
                        >
                              ...
                              >
                                {hsResult ? (
                                  <div className="flex flex-col items-center justify-center gap-1">
                                    ...
                                    {exam.exam_type === "exam" &&
                                      hsResult.cheat_count > 0 && (
                                        <span
                                          className="text-[10px] bg-red-100 text-red-600 px-2 py-0.5 rounded flex items-center gap-1 shadow-sm animate-pulse"
                                          title={`Thoát màn hình ${hsResult.cheat_count} lần`}
                                        >
                                          <AlertTriangle size={10} /> Gian lận:{" "}
                                          {hsResult.cheat_count}
                                        </span>
                                      )}
                                  </div>
                                ) : (
                          ...
                          <td className="p-4 text-right no-print">
                            <button
                              ...
                              className="text-gray-400 hover:text-red-500 bg-white hover:bg-red-50 p-2 rounded-lg transition-colors border border-transparent hover:border-red-100 opacity-0 group-hover:opacity-100"
...
function ExamCreator({ cls, navigate, setExams, exams, supabase, showToast }) {
  ...
  const addQuestion = (type) => {
    ...
    setQuestions([
      ...questions,
      { id: "q_" + Date.now(), type, correct: defaultCorrect, videoUrl: "" },
    ]);
  };
  ...
  const handleSave = async () => {
    if (!title.trim())
      return showToast("Vui lòng nhập tiêu đề bài kiểm tra!", "error");
    if (!fileUploaded || !selectedFile)
      return showToast("Vui lòng đính kèm file đề bài (PDF)!", "error");
    if (questions.length === 0)
      return showToast("Vui lòng thêm ít nhất 1 câu hỏi!", "error");
    ...
    if (uploadError) {
      showToast("Lỗi tải file: " + uploadError.message, "error");
      setIsSaving(false);
      return;
    }
    ...
    const newExam = {
      ...
      maxAttempts:
        examType === "exam" && maxAttempts ? parseInt(maxAttempts) : null,
    };
  ...
  return (
    <div className="space-y-6 pb-20 page-transition">
      <div className="flex justify-between items-center">
        <button
          onClick={() => navigate("class", { cls })}
          className="text-gray-500 font-bold hover:text-black transition-colors flex items-center gap-1 group"
        >
          <ArrowLeft
            size={16}
        ...
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="bg-black text-white px-8 py-3 rounded-xl font-bold hover:bg-gray-800 hover:-translate-y-1 transition-all shadow-md disabled:bg-gray-400"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm transition-shadow hover:shadow-md space-y-6">
          <div>
            <label className="font-black text-sm uppercase text-gray-700">
              1. Tên bài kiểm tra
            </label>
            <input
              type="text"
              className="w-full border-2 border-gray-200 p-4 rounded-xl mt-2 font-bold text-lg outline-none focus:border-black transition-colors bg-gray-50 focus:bg-white"
              value={title}
          ...
          <div>
            ...
            <div className="flex gap-4 mt-2">
              <div
                ...
                className={`flex-1 p-4 rounded-xl border-2 cursor-pointer transition-all ${examType === "practice" ? "border-green-500 bg-green-50" : "border-gray-200 hover:bg-gray-50"}`}
              >
                <div className="flex items-center gap-2 font-black text-green-700 mb-1">
                  <BookOpen size={18} /> Luyện tập tự do
                </div>
                <p className="text-xs font-bold text-gray-500">
                  Học sinh làm bài thoải mái, không bật giám sát.
                </p>
              </div>
              <div
                onClick={() => setExamType("exam")}
                className={`flex-1 p-4 rounded-xl border-2 cursor-pointer transition-all ${examType === "exam" ? "border-purple-500 bg-purple-50" : "border-gray-200 hover:bg-gray-50"}`}
              >
                <div className="flex items-center gap-2 font-black text-purple-700 mb-1">
                  <ShieldAlert size={18} /> Thi nghiêm ngặt
                </div>
                <p className="text-xs font-bold text-gray-500">
                  Giám sát thoát màn hình và cho phép tính giờ.
                </p>
              </div>
            </div>
            ...
            {examType === "exam" && (
              <div className="mt-4 space-y-4 animate-fade-in-down">
                <input
                  type="number"
                  placeholder="Thời gian làm bài (Phút) - Để trống nếu không tính giờ"
                  className="w-full border-2 border-purple-200 p-4 rounded-xl font-bold outline-none focus:border-purple-500 transition-colors bg-purple-50 focus:bg-white text-purple-900 placeholder:text-purple-300"
                  value={duration}
                ...
                <input
                  type="number"
                  placeholder="Số lần làm bài tối đa (VD: 1) - Để trống = Vô hạn"
                  className="w-full border-2 border-purple-200 p-4 rounded-xl font-bold outline-none focus:border-purple-500 transition-colors bg-purple-50 focus:bg-white text-purple-900 placeholder:text-purple-300"
                  value={maxAttempts}
          ...
          <div>
            <label className="font-black text-sm uppercase text-gray-700">
              3. Hạn chót nộp bài (Tùy chọn)
            </label>
            <input
              type="datetime-local"
              className="w-full border-2 border-gray-200 p-4 rounded-xl mt-2 font-bold outline-none focus:border-black transition-colors bg-gray-50 focus:bg-white"
              value={deadline}
          ...
          <div>
            <label className="font-black text-sm uppercase text-gray-700">
              4. Đính kèm File Đề (PDF)
            </label>
            <div
              className={`border-2 border-dashed p-10 text-center mt-2 flex flex-col items-center rounded-2xl transition-colors duration-300 ${fileUploaded ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:border-gray-400 hover:bg-gray-50"}`}
            >
              <input
                type="file"
                accept=".pdf"
              ...
              />
              {fileUploaded && (
                <span className="mt-3 font-bold text-blue-600">
        ...
        <div className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm h-[85vh] flex flex-col transition-shadow hover:shadow-md">
            ...
            <div className="flex flex-wrap gap-4 items-end">
              <div>
                ...
                <input
                  type="number"
                  min="0"
                  value={bulkCounts.mcq4}
                  onChange={(e) =>
                    setBulkCounts({ ...bulkCounts, mcq4: e.target.value })
                  }
                  className="w-20 p-2.5 border-2 border-gray-200 rounded-xl outline-none focus:border-black font-black text-center transition-colors"
                />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 block mb-1">
                ...
                <input
                  ...
                  value={bulkCounts.tf}
                  onChange={(e) =>
                    setBulkCounts({ ...bulkCounts, tf: e.target.value })
                  }
                  className="w-20 p-2.5 border-2 border-gray-200 rounded-xl outline-none focus:border-black font-black text-center transition-colors"
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  min="0"
                  value={bulkCounts.short}
                  onChange={(e) =>
                    setBulkCounts({ ...bulkCounts, short: e.target.value })
                  }
                  className="w-20 p-2.5 border-2 border-gray-200 rounded-xl outline-none focus:border-black font-black text-center transition-colors"
                />
              </div>
              <button
                ...
                className="bg-black text-white px-5 py-2.5 rounded-xl font-bold hover:bg-gray-800 transition-transform hover:scale-105 active:scale-95 h-[46px]"
              >
                TẠO NGAY
              </button>
            </div>
          ...
          <div className="flex gap-2 mb-4 border-b border-gray-100 pb-4">
            <button
              onClick={() => addQuestion("mcq4")}
              className="bg-gray-100 px-4 py-2.5 rounded-xl font-bold text-sm hover:bg-gray-200 hover:-translate-y-0.5 transition-all"
            >
              + Thêm 1 câu A-B-C-D
            </button>
            <button
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-2">
            {questions.length === 0 && (
              <div className="text-center text-gray-400 font-bold mt-10">
                Chưa có câu hỏi nào. Dùng công cụ phía trên để tạo nhé!
              </div>
            )}
            {questions.map((q, i) => (
              <div
                key={q.id}
                className="p-5 bg-gray-50 border border-gray-100 rounded-2xl relative animate-pop"
              >
                <button
                  onClick={() =>
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    {["A", "B", "C", "D"].map((opt) => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-full border-2 font-black transition-all ${q.correct === opt ? "bg-black text-white border-black scale-110 shadow-md" : "border-gray-200 text-gray-400 hover:bg-gray-200 hover:text-gray-600"}`}
                      >
                        {opt}
                      </button>
                    ))}
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    {["Đúng", "Sai"].map((opt) => (
                      <button
                        key={opt}
                        onClick={() => updateQuestion(q.id, "correct", opt)}
                        className={`px-4 py-2 rounded-xl border-2 font-black transition-all ${q.correct === opt ? "bg-black text-white border-black scale-105 shadow-md" : "border-gray-200 text-gray-400 hover:bg-gray-200 hover:text-gray-600"}`}
                      >
                        {opt}
                      </button>
                    ))}
                ...
                {q.type === "short" && (
                  <input
                    type="text"
                    placeholder="Nhập đáp án (Dùng dấu | nếu có nhiều đáp án. VD: A|B)"
                    className="p-3 border-2 border-gray-200 rounded-xl font-bold w-full mb-3 outline-none focus:border-black transition-colors"
                    value={q.correct}
                    onChange={(e) =>
                ...
                <input
                  type="text"
                  placeholder="Link video giải (tùy chọn)"
                  className="p-3 border-2 border-gray-200 rounded-xl text-sm w-full outline-none focus:border-blue-500 transition-colors bg-white"
                  value={q.videoUrl}
                  onChange={(e) =>
                    updateQuestion(q.id, "videoUrl", e.target.value)
                  }
                />
              </div>
            ))}
...
function ExamTaker({
  ...
  useEffect(() => {
    if (currentUser?.role === "student") {
      ...
      const flaggedDraft = localStorage.getItem(
        `flagged_${exam.id}_${currentUser.id}`,
      );
  ...
  const handleSelect = (qId, val) => {
    ...
    if (currentUser?.role === "student") {
      localStorage.setItem(
        `draft_${exam.id}_${currentUser.id}`,
  ...
  const submitExam = async () => {
    ...
    const details = questionsList.map((q) => {
      const userAnswer = String(answers[q.id] || "")
        .toLowerCase()
        .trim();
      const correctAnswers = String(q.correct)
        .toLowerCase()
        .split("|")
        .map((s) => s.trim());
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4 transition-opacity backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 max-w-sm text-center animate-pop shadow-2xl">
            ...
            <p className="font-bold text-gray-600 mb-6">
              Bạn vừa thoát màn hình (Lần {cheatCount}).
              <br />
              Hành động này sẽ bị báo cáo!
            </p>
            <button
              onClick={() => setCheatWarning(false)}
              className="w-full bg-black text-white py-3 rounded-xl font-bold hover:scale-105 transition-transform active:scale-95"
            >
              Quay lại làm bài
            </button>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-gray-200 shadow-sm h-full flex flex-col items-center justify-center text-gray-300 font-bold hover:shadow-md transition-shadow relative overflow-hidden">
            {exam.fileUrl && exam.fileUrl !== "fake-url" ? (
              ...
              <>
                ...
                <p className="mt-4">
                  Đề thi này không có file đính kèm hoặc đã bị giáo viên dọn
                  dẹp.
                </p>
              </>
            )}
        ...
        <div className="w-1/3 bg-white border-l border-gray-200 h-full flex flex-col shadow-2xl relative z-10">
          <div className="p-5 border-b border-gray-200 flex justify-between items-center bg-gray-50 shrink-0">
            <div>
              ...
              {currentUser.role === "teacher" ? (
                <span className="text-xs font-bold text-orange-500 bg-orange-100 px-2 py-0.5 rounded-full mt-1 inline-block">
                  Xem trước (Không lưu)
                </span>
              ) : (
                <div className="flex items-center gap-2 mt-1">
                  {isExamMode ? (
                    <span className="text-[10px] font-bold text-purple-600 bg-purple-100 px-2 py-0.5 rounded flex items-center gap-1 w-max">
                      <ShieldAlert size={12} /> Giám sát
                    </span>
                  ) : (
                    <span className="text-[10px] font-bold text-green-600 bg-green-100 px-2 py-0.5 rounded flex items-center gap-1 w-max">
                      <BookOpen size={12} /> Luyện tập
                    </span>
                  )}

                  {timeLeft !== null && (
                    <span
                      className={`text-[11px] font-black px-2 py-0.5 rounded flex items-center gap-1 w-max transition-colors ${timeLeft < 60 ? "bg-red-100 text-red-600 animate-pulse" : "bg-gray-200 text-gray-800"}`}
                    >
            ...
            <button
              onClick={submitExam}
              className="bg-black text-white px-5 py-2.5 rounded-xl font-black hover:bg-gray-800 hover:-translate-y-0.5 transition-all shadow-md hover:shadow-lg"
            >
              NỘP BÀI
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-gray-50/50">
            {(exam.questions || []).map((q, i) => (
              <div
                key={q.id}
                className={`p-5 border-2 rounded-2xl bg-white shadow-sm transition-colors ${flagged[q.id] ? "border-orange-400 bg-orange-50/30" : "border-gray-100 hover:border-gray-300"}`}
              >
                <div className="flex justify-between items-center mb-4">
                  <h4 className="font-black text-gray-800">Câu {i + 1}</h4>
                  <button
                    onClick={() => toggleFlag(q.id)}
                    className={`p-2 rounded-lg transition-colors flex items-center gap-1.5 text-xs font-bold ${flagged[q.id] ? "bg-orange-100 text-orange-600" : "bg-gray-100 text-gray-400 hover:text-orange-500 hover:bg-orange-50"}`}
                    title="Cắm cờ câu này để xem lại sau"
                  >
                    <Flag
                      size={16}
                      className={flagged[q.id] ? "fill-orange-500" : ""}
                    />{" "}
                    {flagged[q.id] ? "Đã cắm cờ" : "Cắm cờ"}
                  </button>
                </div>
  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto space-y-8 pb-10 page-transition">
        <button
          onClick={() => navigate("class", { cls: { id: exam.classId } })}
          className="font-bold text-gray-500 hover:text-black flex items-center gap-1.5 transition-colors group"
        >
          <ArrowLeft
            size={16}
            className="group-hover:-translate-x-1 transition-transform"
        ...
        <div className="bg-white border border-gray-200 p-12 rounded-[2.5rem] text-center shadow-xl relative overflow-hidden group hover:shadow-2xl transition-shadow">
          ...
          {currentUser.role === "teacher" && (
            <p className="mt-6 font-bold text-orange-500 bg-orange-50 inline-block px-4 py-2 rounded-xl border border-orange-100">
              Bản làm thử (Không lưu vào bảng điểm)
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {(exam.questions || []).map((q, index) => {
            ...
            return (
              <div
                key={q.id}
                className={`bg-white border-2 p-6 rounded-3xl flex flex-col justify-between shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 ${isCorrect ? "border-green-200 bg-green-50/30" : "border-red-200 bg-red-50/30"}`}
              >
                <div>
                  ...
                  <div className="text-base font-medium space-y-2 mb-6">
                    <p className="text-gray-600">
                      Bạn đã chọn:{" "}
                      <span
                        className={`font-black ml-1 ${isCorrect ? "text-green-600" : "text-red-600"}`}
                      >
                        {detail?.userAnswer || "(Không điền)"}
                      </span>
                    </p>
                    {!isCorrect && (
                      <p className="text-gray-600">
                        Đáp án chính xác:{" "}
                        <span className="font-black text-black ml-1 bg-gray-100 px-2 py-1 rounded">
                          {q.correct}
                        </span>
                      </p>
                    )}
                ...
                {q.videoUrl ? (
                  <button
                    onClick={() => setPlayingVideo(q.videoUrl)}
                    className="mt-auto flex items-center justify-center gap-2 bg-blue-50 text-blue-700 border border-blue-200 px-5 py-3 rounded-xl text-sm font-black hover:bg-blue-600 hover:text-white transition-colors duration-300 w-full"
                  >
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 relative">
      <div className="bg-white p-8 sm:p-10 rounded-3xl shadow-2xl w-full max-w-md animate-pop border border-gray-100">
        ...
        <p className="text-gray-500 font-bold text-center mb-8">
          {authMode === "login" ? "Đăng nhập an toàn" : "Tạo tài khoản mới"}
        </p>
        ...
        <div className="space-y-4">
          {authMode === "signup" && (
            <input
              ...
              className="w-full border-2 border-gray-200 p-4 rounded-xl font-bold outline-none focus:border-black bg-gray-50 focus:bg-white transition-colors"
              onChange={(e) =>
          ...
          {authMode === "login" && (
            <div className="flex items-center gap-2 mt-2 px-1">
              <input
                ...
                onChange={(e) =>
                  setAuthForm({ ...authForm, remember: e.target.checked })
                }
              />
              <label
                htmlFor="remember"
          ...
          <button
            onClick={handleAuth}
            className="w-full bg-black text-white p-4 rounded-xl font-black text-lg hover:bg-gray-800 hover:-translate-y-1 transition-all shadow-lg mt-4"
          >
      ...
      <div className="absolute bottom-8 text-center text-sm font-bold text-gray-400 animate-fade-in-down">
        Phát triển bởi{" "}
        <span className="text-gray-800 font-black">Hong Quang Tech</span>
      </div>
    </div>
  );
}
